...
export function resolveSenderForType(type: TransactionalEmailType): {
  ...
  const brandName = process.env.ZEPTO_MAIL_FROM_NAME || 'FR8X';
...
export async function sendTransactionalEmail(
  ...
  if (apiKey && apiKey !== 'undefined') {
    ...
    const payload: Record<string, any> = {
      ...
      client_reference: params.clientReference || correlationId,
    };
...
export async function sendTemplateEmail(
  ...
  // When pre-rendered HTML is provided (and remote unstyled templates are not explicitly forced),
  // dispatch directly via sendTransactionalEmail so that pixel-perfect FR8X branded HTML, CTA buttons,
  // 6-digit codes, and links are delivered with 100% fidelity.
  if (params.fallbackHtml && process.env.ZEPTO_MAIL_PREFER_REMOTE_TEMPLATES !== 'true') {
      ...
      type:
        templateConfig.senderType === 'PASSWORD'
          ...
          : templateConfig.senderType === 'SUPPORT'
            ? 'SUPPORT_REQUEST'
            : 'TECH_NOTIFICATION',
      senderType: templateConfig.senderType,
      to: cleanTo,
    ...
    const finalResult: SendTemplateEmailResult = {
    ...
    if (txResult.success) {
      recordEmailEvent({
        ...
        template: params.template,
        templateKey,
        sender: txResult.from,
        recipient: cleanTo,
        provider: txResult.provider as any,
        providerMessageId: txResult.messageId,
        status: 'sent',
        sentAt: new Date().toISOString(),
      });
      idempotencyCache.set(clientReference, { timestamp: now, result: finalResult });
    }
...
export async function sendEmail(params: SendEmailParams): Promise<SendEmailResponse> {
  ...
  const resolvedFromType: EmailSenderType =
    params.fromType ||
    (rawEvent.includes('SUPPORT')
      ? 'SUPPORT'
      : rawEvent.includes('TECH') || rawEvent.includes('MAINT') || rawEvent.includes('INCIDENT')
        ? 'TECH'
        : 'PASSWORD');
  ...
  if (!isValidEmailAddress(cleanTo)) {
    return {
      ...
      sender: EMAIL_SENDERS[resolvedFromType] || 'unknown',
      to: cleanTo,
  ...
  const flowUrl =
    resolvedFromType === 'PASSWORD' && customPassUrl && customPassUrl.trim() && customPassUrl !== 'undefined'
      ? customPassUrl.trim()
      : (process.env.ZOHO_FLOW_WEBHOOK_URL || '').trim();

  // If a Zoho Flow webhook is configured, dispatch through it (for legacy webhook tests / webhook routing)
  if (flowUrl && flowUrl !== 'undefined') {
    const senderAddress = EMAIL_SENDERS[resolvedFromType] || EMAIL_SENDERS.SUPPORT;
    try {
      const controller = new AbortController();
      const timeoutId = setTimeout(() => controller.abort(), 8000);
      ...
      return {
        ...
        provider: 'ZOHO_FLOW',
        isPasswordConfigured: true,
        error: `Zoho Flow returned HTTP status ${response.status}`,
      };
    } catch (err: any) {
      ...
      return {
        ...
        isPasswordConfigured: true,
        error: isTimeout
          ? 'Zoho Flow webhook request timed out (8s limit exceeded)'
          : `Zoho Flow dispatch network error: ${err.message}`,
      };
    }
  }
...
export const EmailService = {
  ...
  async sendOtpEmail(
    ...
    const targetEmail = (params.recipient || (params as any).to || '').trim();
    const expiryMinutes = params.expiryMinutes || 1;
